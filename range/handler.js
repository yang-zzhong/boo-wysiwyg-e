...
export default class RangeHandler {
  ...
  execCommand(command, arg) {
    ...
    case Command.FONT_SIZE: {
      // 重新实现，改为直接修改样式
      const tns = this.getAllTextNodesInRange();
      if (!tns.length) {
        break;
      }
      if (tns.length === 1 && tns[0] === this.range.startContainer
        && tns[0] === this.range.endContainer) {
        const textNode = tns[0];
